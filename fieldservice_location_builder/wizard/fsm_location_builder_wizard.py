...
class FSMLocationBuilderWizard(models.TransientModel):
    ...
    def create_sub_locations(self):
        for loc in self:
            levels = len(loc.level_ids) - 1
            location = loc.env['fsm.location'].\
                browse(loc.env.context.get('active_id'))

            def build_location(parent, num):
                if loc.level_ids[num].spacer:
                    spacer = " " + loc.level_ids[num].spacer + " "
                else:
                    spacer = " "
                for lev_id in range(loc.level_ids[num].start_number,
                                    loc.level_ids[num].end_number + 1):
                    vals = loc.prepare_fsm_location_values(
                        location, parent, spacer, lev_id, num)
                    new_location = loc.env['fsm.location'].create(vals)
                    if num < levels:
